...
export function WeeklyDashboard(props: Props) {
  ...
  function eventCard(event: ClubEvent, featured = false) {
    ...
    const todayIdea = trainingIdea(event, weekEvents, young, planMeta[event.date]);
    ...
    return <article key={event.id} className={`coach-week-event ${featured ? "featured" : ""} ${event.cancelledAt ? "cancelled" : ""}`} onTouchStart={(e) => { const touch = e.touches[0]; swipeStart.current = { id: event.id, x: touch.clientX, y: touch.clientY }; }} onTouchEnd={(e) => {
      ...
      <button type="button" className="coach-week-event-title" onClick={() => props.onOpenEvent(event.id)}><h3>{trainingTitle(event, planMeta[event.date])}</h3><ChevronRight size={18} /></button>
      ...
      {featured && training && !ended && !event.cancelledAt && <details className="coach-week-today-idea"><summary><Sparkles size={15} /><span>{todayIdea.planned ? "Dein Schwerpunkt" : "Spielidee für heute"}: <strong>{todayIdea.title}</strong></span></summary><p>{todayIdea.text}</p><small>{todayIdea.reason}</small><button type="button" className="coach-week-text-button" onClick={() => props.onBrowseExercises(event.date)}>Übungen auswählen <ArrowRight size={14} /></button></details>}

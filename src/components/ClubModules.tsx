...
import { ageInYears, managedProfileLabel, visibleProfileEmail } from "@/lib/player-profile";
  ...
  return <div className="team-invite-backdrop" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) onClose(); }}>
    <section className="team-invite-dialog" role="dialog" aria-modal="true" aria-labelledby="team-invite-title">
      ...
      {!created ? <form onSubmit={invite}>
        ...
        {accessManagementEnabled && role === "player" && <fieldset className="team-player-kind"><legend>Spielerzugang</legend><div><button type="button" className={playerKind === "child" ? "active" : ""} onClick={() => setPlayerKind("child")}><strong>{managedProfileLabel(birthday, teamAgeGroup)}</strong><small>Ohne eigenen Login</small></button><button type="button" className={playerKind === "account" ? "active" : ""} onClick={() => setPlayerKind("account")}><strong>Eigener Zugang</strong><small>Ab 16 empfohlen · per Einladung</small></button></div></fieldset>}

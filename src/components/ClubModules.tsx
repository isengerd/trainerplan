...
export function TeamPage({ users, invitations, currentUser, accessManagementEnabled, onUsersChange, onProfile, smtpConfigured, onInvited }: { users: ClubUser[]; invitations: ClubInvitation[]; currentUser: ClubUser; accessManagementEnabled: boolean; onUsersChange: (users: ClubUser[]) => void; onProfile: (user: ClubUser) => void; smtpConfigured: boolean; onInvited: () => void }) {
  ...
  return <section className="team-page module-page">
    ...
    {accessManagementEnabled ? <><div className="rights-info"><Shield /><span><strong>Rollen und Rechte</strong><small>Admins verwalten Rollen und Zugänge. Trainer verwalten Termine, Trainings und Teilnahmen.</small></span></div>{players.some((player) => player.managedProfile) && <div className="access-migration-card"><div><KeyRound /><span><strong>Zugänge für vorhandene Spieler einrichten</strong><small>{players.filter((player) => player.managedProfile).length} Kinderprofile können schrittweise mit einem Elternzugang verbunden werden. Es wird nichts automatisch versendet.</small></span></div><button className="primary" onClick={() => setInviteOpen(true)}>Zugänge einrichten</button></div>}</> : <div className="rights-info"><Lock /><span><strong>Mehr mit EM Pro</strong><small>Elternzugänge, Einladungen sowie Rollen und Rechte sind in EM Pro enthalten. Deine Spielerdaten bleiben erhalten.</small></span></div>}
    {inviteOpen && <TeamInviteDialog users={users} invitations={invitations} accessManagementEnabled={accessManagementEnabled} smtpConfigured={smtpConfigured} onClose={() => setInviteOpen(false)} onInvited={onInvited} />}
  </section>;
}

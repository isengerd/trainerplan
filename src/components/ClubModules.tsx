...
function EventDetail({ event, settings, users, currentUser, onRespond, onPrevious, onNext, position, onEdit, onDuplicate, onDelete, onCancel, onClose, onSaveNote, canManage }: { event: ClubEvent; settings: ClubSettings; users: ClubUser[]; currentUser: ClubUser; onRespond: (userId: string, value: Attendance) => void; onPrevious?: () => void; onNext?: () => void; position: { current: number; total: number }; onEdit: () => void; onDuplicate: () => void; onDelete: () => void; onCancel: () => void; onClose: () => void; onSaveNote: (note: string) => void; canManage: boolean }) {
  ...
  useEffect(() => {
    setNote(event.trainerNote ?? "");
    setNoteSaved(false);
    setAttendanceFilter("all");
    setResponseUserId(responseSubjects[0]?.id ?? currentUser.id);
  }, [event.id]);
  ...
  return <div className="modal-backdrop event-popup-backdrop" onMouseDown={onClose}>
    <section className="event-popup" role="dialog" aria-modal="true" aria-labelledby="event-popup-title" onMouseDown={(event) => event.stopPropagation()}>
      ...
      <div className="event-popup-body">
        <div className="event-participation-overview">
          <div>

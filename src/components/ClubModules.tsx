...
import { defaultPosition, eventLabels, positionOptions, roleLabels, type Attendance, type ClubEvent, type ClubInvitation, type ClubSettings, type ClubUser, type EventType, type RepeatFrequency, type Role, type TeamGroup, type TournamentPlan } from "@/data/club";
...
export function TeamPage({ teamAgeGroup, users, invitations, currentUser, accessManagementEnabled, onUsersChange, onProfile, smtpConfigured, onInvited }: { teamAgeGroup?: string; users: ClubUser[]; invitations: ClubInvitation[]; currentUser: ClubUser; accessManagementEnabled: boolean; onUsersChange: (users: ClubUser[]) => void; onProfile: (user: ClubUser) => void; smtpConfigured: boolean; onInvited: () => void }) {
  ...
  const visible = users
    .filter((user) => (accessManagementEnabled || user.role === "player" || user.role === "guardian") && (role === "all" || user.role === role) && user.name.toLowerCase().includes(query.toLowerCase()))
    .sort((a, b) => {
  ...
  return <section className="team-page module-page">
    <div className="module-hero"><div><span className="eyebrow">MANNSCHAFT</span><h1>Unsere Mannschaft</h1>{accessManagementEnabled && <p>Kader, Rollen und Zugänge an einem Ort</p>}</div>{currentUser.role === "admin" && <button className="primary" onClick={() => setInviteOpen(true)}><Plus /> <span>{accessManagementEnabled ? "Einladen" : "Spieler hinzufügen"}</span></button>}</div>
  ...
  return <div className="team-invite-backdrop" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) onClose(); }}>
    <section className="team-invite-dialog" role="dialog" aria-modal="true" aria-labelledby="team-invite-title">
      ...
      {!created ? <form onSubmit={invite}>
        ...
        {role === "player" && <fieldset className="team-player-kind"><legend>Spielerzugang</legend><div><button type="button" className={playerKind === "child" ? "active" : ""} onClick={() => setPlayerKind("child")}><strong>Ohne eigenen Zugang</strong><small>Eltern können mitverwalten</small></button><button type="button" className={playerKind === "account" ? "active" : ""} onClick={() => setPlayerKind("account")}><strong>Eigener Zugang</strong><small>Selbst verwalten · per Einladung</small></button></div></fieldset>}
        ...
        {<PendingInvitations invitations={invitations} users={users} onChanged={onInvited} canResend={accessManagementEnabled} />}
...
export function CalendarPage({ events, plannedTrainings = [], tournamentPlans = [], users, settings, currentUser, selectedEventId, selectedPlannedDate, onSelectedEventHandled, onSelectedPlannedDateHandled, onEventsChange, onDeleteEvent, onDeletePlannedTraining, onOpenTrainingPlan, onOpenTournamentPlanning }: { events: ClubEvent[]; plannedTrainings?: PlannedCalendarTraining[]; tournamentPlans?: TournamentPlan[]; users: ClubUser[]; settings: ClubSettings; currentUser: ClubUser; selectedEventId?: string | null; selectedPlannedDate?: string | null; onSelectedEventHandled?: () => void; onSelectedPlannedDateHandled?: () => void; onEventsChange: (events: ClubEvent[]) => void; onDeleteEvent: (event: ClubEvent, onDeleted: () => void) => void; onDeletePlannedTraining?: (date: string) => void; onOpenTrainingPlan: (date: string) => void; onOpenTournamentPlanning?: (eventId: string) => void }) {
...
function EventDetail({ hasTrainingPlan, onOpenTrainingPlan, event, tournamentPlan, settings, users, currentUser, onRespond, onPrevious, onNext, position, onEdit, onDuplicate, onDelete, onCancel, onClose, onOpenTournamentPlanning, canManage }: { hasTrainingPlan: boolean; onOpenTrainingPlan: () => void; event: ClubEvent; tournamentPlan?: TournamentPlan; settings: ClubSettings; users: ClubUser[]; currentUser: ClubUser; onRespond: (userId: string, value: Attendance) => void; onPrevious?: () => void; onNext?: () => void; position: { current: number; total: number }; onEdit: () => void; onDuplicate: () => void; onDelete: () => void; onCancel: () => void; onClose: () => void; onOpenTournamentPlanning?: () => void; canManage: boolean }) {
  ...
  return <div className="modal-backdrop event-popup-backdrop" onMouseDown={onClose}>
    <section className="event-popup" role="dialog" aria-modal="true" aria-labelledby="event-popup-title" onMouseDown={(event) => event.stopPropagation()}>
      ...
      <div className="event-popup-body">
        ...
        {event.type === "training" && canManage && !event.cancelledAt && <section className="event-training-plan"><div><small>TRAININGSPLANUNG</small><strong>{hasTrainingPlan ? "Trainingsplan für diesen Termin" : "Übungen für dieses Training zusammenstellen"}</strong></div><button type="button" onClick={onOpenTrainingPlan}>{hasTrainingPlan ? "Trainingsplan öffnen" : "Trainingsplan anlegen"}<ChevronRight /></button></section>}
...
export function ProfilePage({ canManageMembership, canViewFamilyAccess, groups, teamAgeGroup, onRemove, user, editable, canChangePassword, canRequestEmailChange, emailChangeByAdmin, canManageAccess, canManageDevelopment, canManagePlayerEquipment, splitTeamsEnabled, onSave, onChangePassword, onBack }: { canManageMembership: boolean; canViewFamilyAccess: boolean; groups: TeamGroup[]; teamAgeGroup?: string; onRemove?: () => Promise<void>; user: ClubUser; editable: boolean; canChangePassword: boolean; canRequestEmailChange: boolean; emailChangeByAdmin: boolean; canManageAccess: boolean; canManageDevelopment: boolean; canManagePlayerEquipment: boolean; splitTeamsEnabled: boolean; onSave: (user: ClubUser) => Promise<void>; onChangePassword: (currentPassword: string, newPassword: string, confirmation: string) => Promise<string | null>; onBack?: () => void }) {
  ...
  useEffect(() => {
    if (!canViewFamilyAccess || user.role !== "player") return;
    void loadFamilyAccess();
  }, [canViewFamilyAccess, user.id, user.managedProfile]);
  ...
  async function loadFamilyAccess() {
    ...
    if (!response.ok) throw new Error(result.error || "Elternzugänge konnten nicht geladen werden.");
    setFamilyAccess({ guardians: result.guardians, invitations: result.invitations }); setForm((current) => ({ ...current, hasGuardianAccess: result.guardians.length > 0 }));
    } catch (error) { setFamilyError(error instanceof Error ? error.message : "Elternzugänge konnten nicht geladen werden."); }
  }

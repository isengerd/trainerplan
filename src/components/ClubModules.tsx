...
export function LoginScreen({ onLogin }: { onLogin: (email: string, password: string) => Promise<string | null> }) {
  const [email, setEmail] = useState("trainer@trainerplan.de");
  ...
  function useDemo(role: Role) {
    const demos: Record<Role, { email: string; password: string }> = {
      admin: { email: "admin@trainerplan.de", password: "admin123" },
      trainer: { email: "trainer@trainerplan.de", password: "trainer123" },
      player: { email: "spieler@trainerplan.de", password: "spieler123" },
    };
  ...
  return <main className="login-page">
    ...
    <section className="login-panel"><form onSubmit={submit}><span className="eyebrow">WILLKOMMEN ZURÜCK</span><h2>Anmelden</h2><p>Nutze einen Demo-Zugang oder deine Anmeldedaten.</p><label><span>E-Mail-Adresse</span><div><Mail /><input type="email" autoComplete="email" value={email} onChange={(event) => setEmail(event.target.value)} /></div></label><label><span>Passwort</span><div><Lock /><input type="password" autoComplete="current-password" value={password} onChange={(event) => setPassword(event.target.value)} /></div></label>{error && <div className="login-error">{error}</div>}<button className="primary login-submit" type="submit" disabled={loading}>{loading ? "Anmeldung läuft …" : <>Anmelden <ChevronRight /></>}</button><div className="demo-divider"><span>Demo-Zugänge</span></div><div className="demo-roles"><button type="button" onClick={() => useDemo("admin")}><Shield />Admin</button><button type="button" onClick={() => useDemo("trainer")}><Users />Trainer</button><button type="button" onClick={() => useDemo("player")}><UserRound />Spieler</button></div></form></section>
  </main>;
}
...
export function ProfilePage({ user, editable, canChangePassword, onSave, onChangePassword, onBack }: { user: ClubUser; editable: boolean; canChangePassword: boolean; onSave: (user: ClubUser) => void; onChangePassword: (currentPassword: string, newPassword: string) => Promise<string | null>; onBack?: () => void }) {
  ...
  function photo(event: React.ChangeEvent<HTMLInputElement>) { const file = event.target.files?.[0]; if (!file) return; const reader = new FileReader(); reader.onload = () => setForm((current) => ({ ...current, avatar: String(reader.result) })); reader.readAsDataURL(file); }
  function saveProfile() { onSave(form); setMessage("Profil gespeichert."); }
  async function changePassword() { if (newPassword.length < 8) return setMessage("Das neue Passwort benötigt mindestens 8 Zeichen."); const error = await onChangePassword(oldPassword, newPassword); if (error) return setMessage(error); setOldPassword(""); setNewPassword(""); setMessage("Passwort geändert."); }
  return <section className="profile-page module-page"><div className="profile-cover" /><div className="profile-heading">{onBack && <button className="profile-back" onClick={onBack}><ChevronLeft /></button>}<div className="profile-photo"><Avatar user={form} size="large" />{editable && <label><Camera /><input type="file" accept="image/*" onChange={photo} /></label>}</div><div><span className={`role-badge ${form.role}`}>{roleLabels[form.role]}</span><h1>{form.name}</h1><p>{form.position} · FC Kicker F1</p></div></div><div className="profile-grid"><section className="profile-card"><span className="eyebrow">PERSÖNLICHE DATEN</span><h2>Profilinformationen</h2><div className="form-row"><label><span>Vor- und Nachname</span><input disabled={!editable} value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} /></label><label><span>E-Mail-Adresse</span><input disabled={!editable} type="email" value={form.email} onChange={(e) => setForm({ ...form, email: e.target.value })} /></label></div><div className="form-row"><label><span>Telefon</span><input disabled={!editable} value={form.phone} onChange={(e) => setForm({ ...form, phone: e.target.value })} /></label><label><span>Geburtsdatum</span><input disabled={!editable} type="date" value={form.birthday} onChange={(e) => setForm({ ...form, birthday: e.target.value })} /></label></div><label><span>Position / Funktion</span><input disabled={!editable} value={form.position} onChange={(e) => setForm({ ...form, position: e.target.value })} /></label>{editable && <button className="primary profile-save" onClick={saveProfile}><Check /> Änderungen speichern</button>}</section>{canChangePassword && <section className="profile-card password-card"><KeyRound /><span className="eyebrow">SICHERHEIT</span><h2>Passwort ändern</h2><p>Verwende mindestens acht Zeichen für dein neues Passwort.</p><label><span>Aktuelles Passwort</span><input type="password" autoComplete="current-password" value={oldPassword} onChange={(e) => setOldPassword(e.target.value)} /></label><label><span>Neues Passwort</span><input type="password" autoComplete="new-password" value={newPassword} onChange={(e) => setNewPassword(e.target.value)} /></label><button onClick={changePassword}>Passwort aktualisieren</button></section>}</div>{message && <div className="profile-message">{message}</div>}</section>;
}

...
export function ProfilePage({ user, editable, canChangePassword, canRequestEmailChange, emailChangeByAdmin, canManageAccess, canManageDevelopment, splitTeamsEnabled, onSave, onChangePassword, onBack }: { user: ClubUser; editable: boolean; canChangePassword: boolean; canRequestEmailChange: boolean; emailChangeByAdmin: boolean; canManageAccess: boolean; canManageDevelopment: boolean; splitTeamsEnabled: boolean; onSave: (user: ClubUser) => void; onChangePassword: (currentPassword: string, newPassword: string, confirmation: string) => Promise<string | null>; onBack?: () => void }) {
  ...
  useEffect(() => {
    if (!canManageAccess || !user.managedProfile) return;
  ...
  async function loadFamilyAccess() {
    ...
    if (response.ok) setFamilyAccess({ guardians: result.guardians, invitations: result.invitations });
  }
  ...
  return <section className="profile-page module-page">
    ...
    </div>{canManageAccess && form.managedProfile && <section className="profile-card managed-access-card"><div className="profile-section-title"><Users /><span><span className="eyebrow">FAMILIE & ZUGÄNGE</span><h2>Eltern und Erziehungsberechtigte</h2><p>Jede Person erhält einen eigenen Elternaccount und handelt für {form.name}.</p></span></div><div className="guardian-access-list">{familyAccess.guardians.map((guardian) => <article key={guardian.id}><span className="guardian-access-avatar">{guardian.name.split(" ").map((part) => part[0]).join("").slice(0, 2)}</span><span><strong>{guardian.name || "Elternteil"}</strong><small>{guardian.email} · Aktiv</small></span><button type="button" onClick={() => void removeFamilyAccess({ guardianId: guardian.id }, guardian.name || guardian.email)} aria-label={`Zugriff von ${guardian.name || guardian.email} entfernen`}><Trash2 /></button></article>)}{familyAccess.invitations.map((invitation) => <article className="pending" key={invitation.id}><span className="guardian-access-avatar"><Mail /></span><span><strong>{invitation.email || "Einladung per Link"}</strong><small>Einladung ausstehend</small></span><button type="button" onClick={() => void removeFamilyAccess({ invitationId: invitation.id }, invitation.email || "dieser Einladung")} aria-label="Einladung zurückziehen"><X /></button></article>)}{familyAccess.guardians.length === 0 && familyAccess.invitations.length === 0 && <p>Noch kein Elternteil verbunden.</p>}</div><div className="guardian-invite-form"><h3>Weiteres Elternteil einladen</h3><label><span>E-Mail-Adresse <small>optional</small></span><input type="email" value={accessEmail} onChange={(event) => setAccessEmail(event.target.value)} placeholder="eltern@beispiel.de" /></label>{accessLink && <div className="access-link-result"><input readOnly value={accessLink} onFocus={(event) => event.currentTarget.select()} /><button onClick={() => void navigator.clipboard.writeText(accessLink)}><Copy /> Kopieren</button></div>}<button className="primary" disabled={emailBusy} onClick={() => void prepareAccess()}><Plus /> Elternteil einladen</button></div></section>}{message && <div className="profile-message">{message}</div>}
  </section>;
}

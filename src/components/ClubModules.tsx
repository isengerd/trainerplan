...
export function TeamPage({ users, invitations, currentUser, accessManagementEnabled, onUsersChange, onProfile, smtpConfigured, onInvited }: { users: ClubUser[]; invitations: ClubInvitation[]; currentUser: ClubUser; accessManagementEnabled: boolean; onUsersChange: (users: ClubUser[]) => void; onProfile: (user: ClubUser) => void; smtpConfigured: boolean; onInvited: () => void }) {
  ...
  return <section className="team-page module-page">
    ...
    {currentUser.role === "admin" && inviteOpen && <TeamInviteDialog users={users} invitations={invitations} accessManagementEnabled={accessManagementEnabled} smtpConfigured={smtpConfigured} onClose={() => setInviteOpen(false)} onInvited={onInvited} />}
  </section>;
}

function TeamInviteDialog({ users, invitations, accessManagementEnabled, smtpConfigured, onClose, onInvited }: { users: ClubUser[]; invitations: ClubInvitation[]; accessManagementEnabled: boolean; smtpConfigured: boolean; onClose: () => void; onInvited: () => void }) {
  ...
  return <div className="team-invite-backdrop" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) onClose(); }}>
    <section className="team-invite-dialog" role="dialog" aria-modal="true" aria-labelledby="team-invite-title">
      ...
      {!created ? <form onSubmit={invite}>
        ...
        {accessManagementEnabled && role === "player" && <fieldset className="team-player-kind"><legend>Spielerzugang</legend><div><button type="button" className={playerKind === "child" ? "active" : ""} onClick={() => setPlayerKind("child")}><strong>Kinderprofil</strong><small>Sofort im Kader</small></button><button type="button" className={playerKind === "account" ? "active" : ""} onClick={() => setPlayerKind("account")}><strong>Eigener Zugang</strong><small>Nur per Einladung</small></button></div></fieldset>}
...
export function CalendarPage({ events, plannedTrainings = [], users, settings, currentUser, selectedEventId, onSelectedEventHandled, onEventsChange, onDeletePlannedTraining }: { events: ClubEvent[]; plannedTrainings?: PlannedCalendarTraining[]; users: ClubUser[]; settings: ClubSettings; currentUser: ClubUser; selectedEventId?: string | null; onSelectedEventHandled?: () => void; onEventsChange: (events: ClubEvent[]) => void; onDeletePlannedTraining?: (date: string) => void }) {
  ...
  return <section className="calendar-page module-page">
    ...
    <div className="calendar-layout"><section className="month-card"><div className="month-head"><button type="button" onClick={() => changeMonth(-1)} aria-label="Vorheriger Monat"><ChevronLeft /></button><h2>{monthLabel}</h2><button type="button" onClick={() => changeMonth(1)} aria-label="Nächster Monat"><ChevronRight /></button></div><div className="month-grid">{["MO", "DI", "MI", "DO", "FR", "SA", "SO"].map((day) => <span className="weekday" key={day}>{day}</span>)}{monthDays.map((day, index) => { const dateKey = day ? `${monthYear}-${String(monthIndex + 1).padStart(2, "0")}-${String(day).padStart(2, "0")}` : ""; const dayEvents = day ? events.filter((event) => event.date === dateKey) : []; const hasPlannedTraining = plannedTrainingByDate.has(dateKey) && !dayEvents.some((event) => event.type === "training"); return <button className={dateKey === selectedDate ? "selected-day" : ""} key={`${monthYear}-${monthIndex}-${index}`} disabled={!day} onClick={(clickEvent) => { if (!day) return; setSelectedDate(dateKey); clickEvent.currentTarget.blur(); openCalendarDay(dateKey, dayEvents); }}>{day}<span>{dayEvents.map((event) => <i className={`${event.type} ${event.cancelledAt ? "cancelled" : ""}`} key={event.id} />)}{hasPlannedTraining && <i className="training" />}</span></button>; })}</div><div className="calendar-legend"><span><i className="training" />Training</span><span><i className="tournament" />Turnier</span><span><i className="event" />Ereignis</span></div></section>
      <section className="upcoming-card"><div className="overview-card-title"><div><span className="eyebrow">ANSTEHEND</span><h2>Nächste Termine</h2></div></div>{upcoming.map((event) => { const yes = Object.values(event.responses).filter((item) => item === "yes").length; return <button className={`${selected?.id === event.id ? "active " : ""}${event.cancelledAt ? "cancelled" : ""}`} key={event.id} onClick={() => openEventDetail(event)}><span className={`event-icon ${event.type}`}>{event.type === "tournament" ? <Trophy /> : <CalendarDays />}</span><span><small>{new Date(`${event.date}T12:00:00`).toLocaleDateString("de-DE", { weekday: "short", day: "2-digit", month: "short" })}</small><strong>{event.title}</strong><p>{event.cancelledAt ? "ABGESAGT" : `${event.startTime} Uhr · ${event.location}`}</p></span><span className="capacity-mini">{event.cancelledAt ? "Absage" : `${yes}/${event.maxParticipants}`}</span></button>; })}</section>
    </div>
    {selected && <EventDetail event={selected} settings={settings} users={users} currentUser={currentUser} onRespond={respond} onEdit={() => { setEditingPlannedDate(null); setEditing({ ...selected }); setSelected(null); }} onDuplicate={() => { setEditingPlannedDate(null); setEditing({ ...selected, id: "", title: `${selected.title} – Kopie`, cancelledAt: null, responses: {} }); setSelected(null); }} onClose={closeEventDetail} onSaveNote={saveTrainerNote} canManage={canManage} onCancel={() => toggleCancellation(selected)} onDelete={() => { if (!window.confirm(`„${selected.title}“ endgültig löschen? Dieser Vorgang kann nicht rückgängig gemacht werden.`)) return; onEventsChange(events.filter((event) => event.id !== selected.id)); closeEventDetail(); }} />}
...
function EventDetail({ event, settings, users, currentUser, onRespond, onEdit, onDuplicate, onDelete, onCancel, onClose, onSaveNote, canManage }: { event: ClubEvent; settings: ClubSettings; users: ClubUser[]; currentUser: ClubUser; onRespond: (userId: string, value: Attendance) => void; onEdit: () => void; onDuplicate: () => void; onDelete: () => void; onCancel: () => void; onClose: () => void; onSaveNote: (note: string) => void; canManage: boolean }) {
      ...
      <div className="event-popup-body">
        ...
        <div className="event-popup-columns">
          <div className="event-main-column">
            {event.cancelledAt ? <section className="event-cancelled-notice"><AlertTriangle /><span><strong>Dieser Termin wurde abgesagt</strong><small>Der Termin bleibt zur Information sichtbar. Eine Teilnahme ist nicht mehr erforderlich.</small></span></section> : settings.attendanceEnabled && responseSubject && <section className="response-card"><header><div><span>DEINE RÜCKMELDUNG</span><h3>{responseSubject.id !== currentUser.id ? `Kommt ${responseSubject.name}?` : "Bist du dabei?"}</h3></div>{responseSubjects.length > 1 && <select aria-label="Person auswählen" value={responseSubject.id} onChange={(event) => setResponseUserId(event.target.value)}>{responseSubjects.map((person) => <option key={person.id} value={person.id}>{person.id === currentUser.id ? `${person.name} (ich)` : person.name}</option>)}</select>}</header><div className="response-options"><button className={event.responses[responseSubject.id] === "yes" ? "yes active" : "yes"} disabled={full || (responseClosed && !canManage)} onClick={() => onRespond(responseSubject.id, "yes")}><ThumbsUp /><span><strong>Dabei</strong><small>Ich komme</small></span></button><button className={event.responses[responseSubject.id] === "maybe" ? "maybe active" : "maybe"} disabled={(responseClosed && !canManage) || isOnlyResponsibleTrainer} onClick={() => onRespond(responseSubject.id, "maybe")}><span className="response-question">?</span><span><strong>Unsicher</strong><small>Noch offen</small></span></button><button className={event.responses[responseSubject.id] === "no" ? "no active" : "no"} disabled={(responseClosed && !canManage) || isOnlyResponsibleTrainer} onClick={() => onRespond(responseSubject.id, "no")}><ThumbsDown /><span><strong>Absage</strong><small>Ich kann nicht</small></span></button></div><footer className={responseClosed ? "closed" : ""}><Clock3 /><span>{isOnlyResponsibleTrainer ? "Als einziger verantwortlicher Trainer bist du fest eingeplant." : responseClosed ? `Rückmeldung geschlossen · Frist war ${deadline.toLocaleString("de-DE", { day: "2-digit", month: "2-digit", hour: "2-digit", minute: "2-digit" })} Uhr` : full ? "Alle Plätze sind aktuell belegt." : `Änderbar bis ${deadline.toLocaleString("de-DE", { weekday: "short", day: "2-digit", month: "2-digit", hour: "2-digit", minute: "2-digit" })} Uhr`}</span></footer></section>}
          ...
          <aside className="event-attendance-column">
            <div className="capacity-block"><div><span>TEILNAHME</span><strong>{counts.yes} / {event.maxParticipants} Plätze</strong></div><div className="attendance-distribution" aria-label={`${counts.yes} dabei, ${counts.maybe + unanswered} offen, ${counts.no} Absagen`}><i className="yes" style={{ width: `${players.length ? counts.yes / players.length * 100 : 0}%` }} /><i className="maybe" style={{ width: `${players.length ? (counts.maybe + unanswered) / players.length * 100 : 0}%` }} /><i className="no" style={{ width: `${players.length ? counts.no / players.length * 100 : 0}%` }} /></div><div className="attendance-counts"><span className="yes">● {counts.yes} dabei</span><span className="maybe">● {counts.maybe + unanswered} offen</span><span className="no">● {counts.no} nicht dabei</span></div></div>
...
function EventEditor({ event, plannedTraining = false, settings, users, onClose, onSave, onDelete }: { event: ClubEvent; plannedTraining?: boolean; settings: ClubSettings; users: ClubUser[]; onClose: () => void; onSave: (event: ClubEvent) => void; onDelete?: () => void }) {
  ...
  return <div className="modal-backdrop event-editor-backdrop" onMouseDown={onClose}><form className="event-editor" onSubmit={submit} onMouseDown={(e) => e.stopPropagation()}>
    ...
    <section className="event-editor-section"><header><Users /><span><strong>Teilnahme</strong><small>Wer wird eingeplant?</small></span></header><div className="event-audience"><Check /><span><strong>Alle aktiven Spieler</strong><small>{players} Spieler erhalten den Termin und können {settings.attendanceEnabled ? "zu- oder absagen" : "den Termin ansehen"}.</small></span></div>{settings.attendanceEnabled && form.type !== "event" && <label className="event-attendance-preset"><span><strong>Alle zunächst als anwesend eintragen</strong><small>Opt-out: Spieler sind eingeplant und sagen nur ab, wenn sie nicht kommen. Ohne diese Auswahl gelten die persönlichen Anwesenheits-Standards der Spieler.</small></span><input type="checkbox" checked={Boolean(form.autoSetPlayersPresent)} onChange={(event) => setForm((current) => ({ ...current, autoSetPlayersPresent: event.target.checked, maxParticipants: event.target.checked ? Math.max(current.maxParticipants, players) : current.maxParticipants }))} /><i /></label>}<div className="form-row"><label><span>Teilnehmerlimit</span><input type="number" inputMode="numeric" min="1" max="99" required value={form.maxParticipants} onChange={(e) => set("maxParticipants", Number(e.target.value))} /></label><div className="event-setting-summary"><Clock3 /><span><strong>Rückmeldefrist</strong><small>{settings.attendanceEnabled && deadline ? `${deadline.toLocaleString("de-DE", { weekday: "short", day: "2-digit", month: "2-digit", hour: "2-digit", minute: "2-digit" })} Uhr` : "Zu-/Absagen sind deaktiviert"}</small></span></div></div></section>
...
export function ProfilePage({ user, editable, canChangePassword, canRequestEmailChange, emailChangeByAdmin, canManageAccess, canManageDevelopment, splitTeamsEnabled, onSave, onChangePassword, onBack }: { user: ClubUser; editable: boolean; canChangePassword: boolean; canRequestEmailChange: boolean; emailChangeByAdmin: boolean; canManageAccess: boolean; canManageDevelopment: boolean; splitTeamsEnabled: boolean; onSave: (user: ClubUser) => void; onChangePassword: (currentPassword: string, newPassword: string, confirmation: string) => Promise<string | null>; onBack?: () => void }) {
  ...
  async function prepareAccess() {
    ...
    try {
      ...
      setAccessLink(result.link); setMessage("Persönlicher Zugangslink erstellt. Es wurde noch nichts automatisch versendet.");
    } finally { setEmailBusy(false); }
  }
  ...
  return <section className="profile-page module-page">
    ...
    </div>{canManageAccess && form.managedProfile && <section className="profile-card managed-access-card"><div className="profile-section-title"><KeyRound /><span><span className="eyebrow">ZUGANG</span><h2>Elternzugang einrichten</h2><p>Optional eine E-Mail hinterlegen oder nur einen persönlichen Link erzeugen.</p></span></div><label><span>E-Mail-Adresse <small>optional</small></span><input type="email" value={accessEmail} onChange={(event) => setAccessEmail(event.target.value)} placeholder="eltern@beispiel.de" /></label>{accessLink && <div className="access-link-result"><input readOnly value={accessLink} onFocus={(event) => event.currentTarget.select()} /><button onClick={() => void navigator.clipboard.writeText(accessLink)}><Copy /> Kopieren</button></div>}<button className="primary" disabled={emailBusy} onClick={() => void prepareAccess()}><KeyRound /> Zugang vorbereiten</button></section>}{message && <div className="profile-message">{message}</div>}
  </section>;
}

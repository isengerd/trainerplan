...
type InvitationInfo = { ownershipTransfer?: boolean; email: string; name: string; role: "admin" | "trainer" | "player" | "guardian"; group?: string; team?: string; club?: string; managedPlayer?: string; expiresAt: string };
...
export function InvitationAccept({ token }: { token: string }) {
  ...
  async function accept(event: React.FormEvent) {
    ...
    try {
      ...
      const response = await fetch("/api/v1/invitations/accept", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(credential ? { token, email, confirmOwnership, idToken: credential.idToken } : { token, email, password }) });
  ...
  const role = info?.ownershipTransfer ? "Mannschaftsinhaber und Trainer" : info?.role === "admin" ? "Admin" : info?.role === "trainer" ? "Trainer" : info?.role === "guardian" ? "Elternteil" : "Spieler";
  ...
  return <main className="login-page invitation-page">
    ...
    <section className="login-panel invitation-panel"><form className="invitation-accept-form" onSubmit={accept}>
      ...
      {loading && !info ? <div className="invitation-loading">Einladung wird geprüft …</div> : info ? <>
        ...
        {info.ownershipTransfer && <label className="ownership-confirm"><input type="checkbox" required checked={confirmOwnership} onChange={(event) => setConfirmOwnership(event.target.checked)} /><span>Ich übernehme die Inhaberschaft und Lizenzverwaltung für {info.club}. In EM Free übernehme ich auch den einzigen Trainerplatz. Bei einer Vereinslizenz gilt die Übergabe für alle Mannschaften.</span></label>}
        ...
        <button className="primary login-submit invitation-submit" type="submit" disabled={loading || (info.ownershipTransfer && !confirmOwnership) || !email || password.length < 12 || !passwordsMatch}>{loading ? "Einladung wird angenommen …" : <><Check /> {info.ownershipTransfer ? "Inhaberschaft übernehmen" : "Einladung annehmen"}</>}</button>

...
export function InvitationAccept({ token }: { token: string }) {
  ...
  async function accept(event: React.FormEvent) {
    ...
    try {
      ...
      const response = await fetch("/api/v1/invitations/accept", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(credential ? { token, name, idToken: credential.idToken } : { token, name, password }) });
  ...
  return <main className="login-page invitation-page">
    <section className="login-brand"><span className="brand-mark"><Users /></span><span className="eyebrow">NEXTSESSION KIDS!</span><h1>Willkommen<br />im Team.</h1><p>Lege dein persönliches Passwort fest und starte direkt mit deiner Mannschaft.</p></section>

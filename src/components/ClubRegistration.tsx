...
export function ClubRegistration() {
  ...
  async function createAccount(event: React.FormEvent) {
    ...
    const response = await fetch("/api/v1/auth/register", { method: "POST", credentials: "include", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ email, password }) });
    const result = await response.json() as { error?: string };
    if (!response.ok) { setError(result.error || "Der Account konnte nicht erstellt werden."); setLoading(false); return; }

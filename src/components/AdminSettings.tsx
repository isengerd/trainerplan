...
export function AdminSettingsPage({ settings, currentUser, users, groups, ageGroups, invitations, smtp, push, organization, onSave, onUsersChange, onReload }: Props) {
  ...
  const [invite, setInvite] = useState<{ name: string; email: string; role: Role; sendEmail: boolean }>({ name: "", email: "", role: "player", sendEmail: smtp.configured });
  const [advancedInvite, setAdvancedInvite] = useState(false);
  ...
  async function createInvite(event: React.FormEvent) {
    ...
    const invitationRole: Role = advancedInvite ? (invite.role === "player" ? "trainer" : invite.role) : "player";
    const response = await fetch("/api/v1/invitations", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ ...invite, role: invitationRole, groupId: null }) });
  ...
  return <section className="settings-page module-page">
    ...
    <div className="settings-layout admin-settings-layout">
      ...
      <section className="settings-card settings-wide"><div className="settings-title"><Shield /><span><h2>Rollen & Rechte</h2><p>Die Mannschaft bestimmt den sportlichen Kontext. Das Geburtsdatum liefert nur einen Altersklassen-Hinweis.</p></span></div><div className="role-matrix">{(["admin", "trainer", "player"] as Role[]).map((role) => <article key={role} className={role}><span className={`role-badge ${role}`}>{roleLabels[role]}</span>{roleDescriptions[role].map((right) => <small key={right}><Check /> {right}</small>)}</article>)}</div><div className="member-rights-table"><div className="member-rights-head"><span>Person</span><span>Mannschaft</span><span>Funktionsgruppe</span><span>Altersklassen-Hinweis</span><span>Rolle</span></div>{users.map((user) => <div key={user.id}><span><strong>{user.name}</strong><small>{user.id === currentUser.id ? `${user.email} · Du selbst` : user.email}</small></span><span className="member-team-name">{form.teamName}</span><select value={user.groupId || ""} onChange={(event) => updateMembership(user.id, { groupId: event.target.value || null })}><option value="">Keine</option>{groups.map((group) => <option key={group.id} value={group.id}>{group.name}</option>)}</select><span className="member-age-group">{user.role === "player" ? user.ageGroup || "Geburtsdatum fehlt" : "—"}</span><select value={user.role} disabled={user.id === currentUser.id} title={user.id === currentUser.id ? "Die eigene Adminrolle kann nicht geändert werden." : "Rolle ändern"} onChange={(event) => updateMembership(user.id, { role: event.target.value as Role })}>{(["player", "trainer", "admin"] as Role[]).map((role) => <option key={role} value={role}>{roleLabels[role]}</option>)}</select></div>)}</div></section>

      {organization?.isClubAdmin && <section className="settings-card settings-wide organization-settings"><div className="settings-title"><Users /><span><h2>Lizenz & Mannschaften</h2><p>Die Einzelmannschaft bleibt vollständig nutzbar. Im Vereinsmodus verwaltest du mehrere Mannschaften unter demselben Verein.</p></span></div><div className="license-status"><span><strong>{organization.licenseType === "club" ? "Vereinslizenz" : "Einzelmannschaftslizenz"}</strong><small>{organization.licenseType === "club" ? `${organization.teams.length} Mannschaften können gemeinsam verwaltet werden.` : "Eine Mannschaft mit allen Trainings-, Kalender- und Teamfunktionen."}</small></span>{organization.licenseType === "single_team" && <button className="primary" type="button" disabled={busy} onClick={enableClubMode}>Auf Vereinslizenz upgraden</button>}</div><div className="organization-team-list">{organization.teams.map((team) => <article key={team.id}><span><strong>{team.name}</strong><small>{ageGroups.find((age) => age.id === team.ageGroup)?.name ?? team.ageGroup} · {team.memberCount} Mitglieder</small></span>{team.id === organization.activeTeamId && <em>Aktiv</em>}</article>)}</div>{organization.licenseType === "club" && <form className="create-organization-team" onSubmit={createTeam}><label><span>Name der Mannschaft</span><input required maxLength={120} value={newTeam.name} onChange={(event) => setNewTeam({ ...newTeam, name: event.target.value })} placeholder="z. B. E1 Grün" /></label><label><span>Altersklasse</span><select value={newTeam.ageGroup} onChange={(event) => setNewTeam({ ...newTeam, ageGroup: event.target.value })}>{activeAgeGroups.map((age) => <option key={age.id} value={age.id}>{age.name}</option>)}</select></label><button className="primary" disabled={busy}><Plus /> Mannschaft anlegen</button></form>}</section>}
      ...
      <section className="settings-card invitation-card"><div className="settings-title"><UserPlus /><span><h2>Spieler einladen</h2><p>Der Zugang wird automatisch deiner Mannschaft „{form.teamName}“ zugeordnet.</p></span></div><form className="invite-form" onSubmit={createInvite}><div className="invite-fields invite-fields-simple"><label><span>Name <small>optional</small></span><input value={invite.name} onChange={(event) => setInvite({ ...invite, name: event.target.value })} placeholder="Vor- und Nachname" /></label><label><span>E-Mail-Adresse</span><input required type="email" value={invite.email} onChange={(event) => setInvite({ ...invite, email: event.target.value })} placeholder="name@verein.de" /></label></div><button className="invite-advanced-toggle" type="button" aria-expanded={advancedInvite} onClick={() => setAdvancedInvite((current) => !current)}>{advancedInvite ? "Als Spieler einladen" : "Trainer oder Admin einladen"}</button>{advancedInvite && <label className="invite-role-field"><span>Rolle mit zusätzlichen Rechten</span><select value={invite.role === "player" ? "trainer" : invite.role} onChange={(event) => setInvite({ ...invite, role: event.target.value as Role })}>{(["trainer", "admin"] as Role[]).map((role) => <option value={role} key={role}>{roleLabels[role]}</option>)}</select></label>}<p className="invite-age-hint">Das Geburtsdatum wird später im Profil erfasst und dient nur als Altersklassen-Hinweis. Es verändert die Mannschaftszuordnung nicht.</p><label className="send-mail-toggle"><input type="checkbox" checked={invite.sendEmail} onChange={(event) => setInvite({ ...invite, sendEmail: event.target.checked })} /><span><Mail /><strong>Einladung per E-Mail senden</strong><small>{smtp.configured ? "SMTP ist bereit. Der Link wird zusätzlich angezeigt." : "SMTP fehlt – du kannst den Link trotzdem kopieren."}</small></span></label><button className="primary invite-submit" disabled={busy}><Send /> {busy ? "Einladung wird erstellt …" : `${advancedInvite ? roleLabels[invite.role === "player" ? "trainer" : invite.role] : "Spieler"} einladen`}</button></form>{inviteLink && <div className="generated-link"><Link2 /><span><strong>Einladungslink ist bereit</strong><input readOnly value={inviteLink} onFocus={(event) => event.currentTarget.select()} /></span><button onClick={() => copyLink(inviteLink)}><Link2 /> Link kopieren</button></div>}</section>

...
import { AlertTriangle, Bell, CalendarDays, Check, Clock3, CreditCard, Download, FileText, Mail, Moon, Palette, Plus, RefreshCw, Server, Settings, Shield, Sun, Trash2, Trophy, Users } from "lucide-react";
...
export function LicensePage({ organization, ageGroups, onReload }: { organization: OrganizationContext; ageGroups: AgeGroupOption[]; onReload: () => void }) {
  ...
  async function upgrade(licenseType: "single_team_pro" | "club") {
    ...
    setMessage(licenseType === "club" ? "Vereinslizenz aktiviert." : "EM Pro aktiviert. Zugänge können jetzt eingerichtet werden."); onReload();
  }
  ...
  return <section className="settings-page module-page license-page">
    ...
    <div className="settings-layout">
      <section className="settings-card settings-wide license-overview"><div className="settings-title"><CreditCard /><span><h2>Aktueller Tarif</h2><p>Der Lizenzinhaber verwaltet den Vertrag unabhängig von den Mannschaftsrollen.</p></span></div><div className="license-status"><span><strong>{organization.licenseType === "club" ? "Vereinslizenz" : organization.licenseType === "single_team_pro" ? "Einzelmannschaft Pro" : "Einzelmannschaft Free"}</strong><small>{organization.licenseType === "club" ? `${organization.teams.length} aktive Mannschaften` : organization.licenseType === "single_team_pro" ? "Zugänge, Einladungen und Rollen für eine Mannschaft" : "Kader und Spielerprofile für eine Mannschaft"}</small></span>{organization.licenseType === "single_team_free" && <button className="primary" disabled={busy} onClick={() => void upgrade("single_team_pro")}>Auf EM Pro upgraden</button>}{organization.licenseType === "single_team_pro" && <button className="primary" disabled={busy} onClick={() => void upgrade("club")}>Auf Vereinslizenz erweitern</button>}</div>{organization.licenseExpiresAt && <div className="license-expiry-warning"><AlertTriangle /><span><strong>Lizenz endet am {new Date(organization.licenseExpiresAt).toLocaleDateString("de-DE")}</strong><small>Danach wird EM Free aktiv. Zugänge werden gesperrt, die gespeicherten Daten bleiben erhalten.</small></span></div>}</section>
      ...
      {organization.licenseType !== "single_team_free" && <section className="settings-card settings-wide license-downgrade"><div className="settings-title"><AlertTriangle /><span><h2>Tarif herabstufen</h2><p>{organization.licenseType === "club" ? `„${activeTeam?.name}“ bleibt aktiv; weitere Mannschaften werden archiviert.` : "Einladungen, Rollenverwaltung und Benutzerzugänge werden gesperrt."} Keine Mannschafts- oder Profildaten werden gelöscht.</p></span></div><div className="license-loss-list"><strong>Nach dem Wechsel nicht mehr verfügbar:</strong><span>• neue Einladungen und Zugänge</span><span>• Rollen- und Rechteverwaltung</span>{organization.licenseType === "club" && <span>• mehrere aktive Mannschaften</span>}</div><label><span>Zur Bestätigung TARIF WECHSELN eingeben</span><input value={confirmation} onChange={(event) => setConfirmation(event.target.value)} /></label><div className="profile-inline-actions">{organization.licenseType === "club" && <button disabled={busy || confirmation !== "TARIF WECHSELN"} onClick={() => void downgrade("single_team_pro")}>Zu EM Pro</button>}<button className="danger" disabled={busy || confirmation !== "TARIF WECHSELN"} onClick={() => void downgrade("single_team_free")}>Zu EM Free</button></div></section>}
    </div>

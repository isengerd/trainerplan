...
import { Bell, Check, Clipboard, Clock3, Link2, Mail, Moon, Palette, Plus, RefreshCw, Send, Server, Shield, Sun, Trash2, UserPlus, Users } from "lucide-react";
import { roleLabels, type ClubInvitation, type ClubSettings, type ClubUser, type Role, type SmtpStatus, type TeamGroup } from "@/data/club";
...
const roleDescriptions: Record<Role, string[]> = {
  ...
  trainer: ["Trainings und Übungen", "Termine und Teilnahmen", "Keine Systemeinstellungen"],
  player: ["Termine ansehen", "Eigene Teilnahme melden", "Eigenes Profil bearbeiten"],
};

export function AdminSettingsPage({ settings, users, groups, invitations, smtp, onSave, onUsersChange, onReload }: Props) {
  const [form, setForm] = useState(settings);
  const [groupForm, setGroupForm] = useState(groups);
  const [invite, setInvite] = useState<{ name: string; email: string; role: Role; groupId: string; sendEmail: boolean }>({ name: "", email: "", role: "player", groupId: groups[0]?.id || "", sendEmail: smtp.configured });
  ...
  function updateMembership(userId: string, patch: Partial<Pick<ClubUser, "role" | "groupId">>) {
  ...
  async function renewInvitation(item: ClubInvitation) {
    setBusy(true);
    const response = await fetch("/api/v1/invitations", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ email: item.email, name: item.name, role: item.role, groupId: item.groupId || null, sendEmail: false }) });
  ...
  return <section className="settings-page module-page">
    ...
    <div className="settings-layout admin-settings-layout">
      ...
      <section className="settings-card invitation-card"><div className="settings-title"><UserPlus /><span><h2>Person einladen</h2><p>Per E-Mail versenden oder Link direkt teilen.</p></span></div><form className="invite-form" onSubmit={createInvite}><div className="form-row"><label><span>Name (optional)</span><input value={invite.name} onChange={(event) => setInvite({ ...invite, name: event.target.value })} /></label><label><span>E-Mail-Adresse</span><input required type="email" value={invite.email} onChange={(event) => setInvite({ ...invite, email: event.target.value })} /></label></div><div className="form-row"><label><span>Rolle</span><select value={invite.role} onChange={(event) => setInvite({ ...invite, role: event.target.value as Role })}>{(["player", "trainer", "admin"] as Role[]).map((role) => <option value={role} key={role}>{roleLabels[role]}</option>)}</select></label><label><span>Gruppe</span><select value={invite.groupId} onChange={(event) => setInvite({ ...invite, groupId: event.target.value })}><option value="">Ohne Gruppe</option>{groups.map((group) => <option value={group.id} key={group.id}>{group.name}</option>)}</select></label></div><label className="send-mail-toggle"><input type="checkbox" checked={invite.sendEmail} onChange={(event) => setInvite({ ...invite, sendEmail: event.target.checked })} /><span><Mail /><strong>Einladung zusätzlich per E-Mail senden</strong><small>{smtp.configured ? "SMTP ist bereit." : "SMTP fehlt – der Link wird trotzdem erstellt."}</small></span></label><button className="primary invite-submit" disabled={busy}><Send /> Einladung erstellen</button></form>{inviteLink && <div className="generated-link"><Link2 /><input readOnly value={inviteLink} onFocus={(event) => event.currentTarget.select()} /><button onClick={() => copyLink(inviteLink)}><Clipboard /> Kopieren</button></div>}</section>

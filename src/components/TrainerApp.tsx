...
type BootstrapData = {
  ...
  users: ClubUser[];
  events: ClubEvent[];
  familyEvents?: FamilyDashboardEvent[];
  exercises: Exercise[];
...
export function TrainerApp() {
  ...
  useEffect(() => {
    ...
    window.history.replaceState({ ...window.history.state, nextSessionView: "overview" }, "", url);
  ...
  async function syncEvents(next: ClubEvent[]) {
    ...
    try {
      ...
      for (const event of next) {
        ...
        const responseSubjects = currentUserId ? (canManageClub ? users.filter((user) => user.role === "player").map((user) => user.id) : [currentUserId, ...(currentUser?.managedPlayerIds ?? [])]) : [];
  ...
  async function updateTournamentPlan(eventId: string, squads: TournamentSquad[]) {
    ...
    const next = [...previous.filter((plan) => plan.eventId !== eventId), { eventId, squads, publishedAt: null }];
  ...
  async function updateTournamentPlanPublication(eventId: string, published: boolean) {
    try {
      const response = await fetch(`/api/v1/events/${encodeURIComponent(eventId)}/squads`, { method: "PATCH", credentials: "include", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ published }) });
      const result = await response.json().catch(() => ({})) as { error?: string; publishedAt?: string | null };
      if (!response.ok) throw new Error(result.error ?? "Freigabe konnte nicht geändert werden.");
      setTournamentPlans((current) => current.map((plan) => plan.eventId === eventId ? { ...plan, publishedAt: result.publishedAt ?? null } : plan));
      showToast(published ? "Mannschaftsplanung für Spieler freigegeben." : "Freigabe zurückgenommen.");
      return true;
    } catch (error) {
      showToast(error instanceof Error ? error.message : "Freigabe konnte nicht geändert werden.");
      return false;
    }
  }
  ...
  const visibleFamilyEvents = selectedManagedPlayerId
    ? familyEvents.filter((event) => event.playerIds.includes(selectedManagedPlayerId))
    : familyEvents;
  const familyDashboardEntries = [...visibleFamilyEvents.reduce<Map<string, { event: FamilyDashboardEvent; variants: FamilyDashboardEvent[]; playerIds: Set<string>; teamNames: Set<string> }>>((grouped, event) => {
    const key = [event.type, event.title.trim().toLocaleLowerCase("de-DE"), event.date, event.meetingTime, event.startTime, event.endTime, event.location.trim().toLocaleLowerCase("de-DE"), Boolean(event.cancelledAt)].join("|");
    const existing = grouped.get(key);
    if (existing) {
      existing.variants.push(event);
      event.playerIds.forEach((id) => existing.playerIds.add(id));
      existing.teamNames.add(event.teamName);
  ...
  const openFamilyEventDetails = async (entry: { variants: FamilyDashboardEvent[] }) => {
    const target = entry.variants.find((event) => event.teamId === organization?.activeTeamId) ?? entry.variants[0];
    if (target.teamId !== organization?.activeTeamId) await switchTeam(target.teamId);
    setCalendarFocusId(target.id);
    setView("calendar");
  };
  ...
  const overview = (
    <section className="overview-page">
      ...
      {currentUser?.role === "guardian" && <section className="overview-card family-dashboard-card"><div className="overview-card-title"><div><span className="eyebrow">FAMILIENKALENDER</span><h2>{selectedManagedPlayer ? `Als Nächstes für ${selectedManagedPlayer.name}` : "Alle Kinder auf einen Blick"}</h2></div></div><div className="family-dashboard-events">{visibleFamilyDashboardEntries.map((entry) => {
  ...
  const moduleContent = view === "calendar"
    ...
    : view === "team"
      ...
      : view === "profile" && profileUser
        ? <ProfilePage key={profileUser.id} canViewFamilyAccess={currentUser.role === "admin"} groups={groups} teamAgeGroup={organization?.teams.find((team) => team.id === organization.activeTeamId)?.ageGroup ?? clubSettings.teamAgeGroup} onRemove={currentUser.role === "admin" && profileUser.role === "player" && profileUser.id !== currentUser.id ? () => removePlayerFromTeam(profileUser) : undefined} user={profileUser} editable={profileUser.id === currentUser.id || currentUser.role === "admin"} canChangePassword={!profileUser.managedProfile && profileUser.id === currentUser.id} canRequestEmailChange={!profileUser.managedProfile && (profileUser.id === currentUser.id || currentUser.role === "admin")} emailChangeByAdmin={currentUser.role === "admin" && profileUser.id !== currentUser.id} canManageMembership={currentUser.role === "admin" && accessManagementEnabled} canManageAccess={currentUser.role === "admin"} canManageDevelopment={canManageClub} canManagePlayerEquipment={Boolean(canManageClub && profileUser.role === "player")} splitTeamsEnabled={clubSettings.splitTeamsEnabled} onSave={updateUser} onChangePassword={changePassword} onBack={profileUser.id !== currentUser.id ? () => setView("team") : undefined} />
        : view === "license" && organization?.canManageLicense
          ? <LicensePage organization={organization} ageGroups={ageGroups} onReload={() => void loadBootstrap()} />
        : view === "settings"
          ? currentUser.role === "admin"
            ? <AdminSettingsPage onOpenTeam={() => setView("team")} settings={clubSettings} groups={groups} ageGroups={ageGroups} smtp={smtp} push={push} organization={organization} onSave={updateSettings} onReload={() => void loadBootstrap()} />
      ...
      <section className="workspace" hidden={templateOpen && canManageClub && mobileTemplatePage}>
          ...
          <div className="top-actions">
            ...
            <div className="account-menu-wrap">
              ...
              {accountMenuOpen && <div className="account-menu" role="menu" onMouseDown={(event) => event.stopPropagation()}>
                ...
                {organization?.canManageLicense && <button role="menuitem" onClick={() => { setAccountMenuOpen(false); setView("license"); }}><CreditCard /><span>Lizenz & Abrechnung</span><ChevronRight /></button>}
        ...
        {mobileMenuOpen && <div className="mobile-menu-backdrop" onMouseDown={() => setMobileMenuOpen(false)}><nav className="mobile-menu-sheet" aria-label="Mobile Hauptnavigation" onMouseDown={(event) => event.stopPropagation()}>
          ...
          <div>
            ...
            {organization?.canManageLicense && <button className={view === "license" ? "active" : ""} onClick={() => mobileNavigate("license")}><CreditCard /><span><strong>Lizenz & Abrechnung</strong><small>Tarif, Zahlung und Rechnungen</small></span><ChevronRight /></button>}
        ...
        </div> : canManageClub ? exerciseDatabase : overview)}

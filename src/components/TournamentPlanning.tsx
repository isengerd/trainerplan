...
function MatchDayOverview({ event, squads, published, users, settings, ageGroups, busy, onCreateTournament, onPublicationChange, onSave }: { event: ClubEvent; squads: TournamentSquad[]; published: boolean; users: ClubUser[]; settings: ClubSettings; ageGroups: AgeGroupOption[]; busy: boolean; onCreateTournament: () => void; onPublicationChange: (published: boolean) => Promise<boolean>; onSave: (squads: TournamentSquad[]) => Promise<boolean> }) {
  ...
  return <section className={`matchday-view ${editing ? "is-editing" : ""}`}>
    ...
    {editingSquadId && draft.find((squad) => squad.id === editingSquadId) && <SquadEditor squad={draft.find((squad) => squad.id === editingSquadId)!} squads={draft} players={playerList} trainers={trainerList} ageGroups={ageGroups} event={event} settings={settings} busy={busy} onClose={() => setEditingSquadId(null)} onSave={(squad) => { updateSquad(squad.id, squad); setEditingSquadId(null); }} />}
  </section>;
}
...
function SquadEditor({ squad, squads, players, trainers, ageGroups, event, settings, busy, onClose, onSave }: { squad: TournamentSquad; squads: TournamentSquad[]; players: ClubUser[]; trainers: ClubUser[]; ageGroups: AgeGroupOption[]; event: ClubEvent; settings: ClubSettings; busy: boolean; onClose: () => void; onSave: (squad: TournamentSquad) => void }) {
  ...
  const responseOrder = { yes: 0, maybe: 1, no: 3 } as const;
  const visible = players
    .filter((player) => (activeAgeNames.has(player.ageGroup) || form.playerIds.includes(player.id)) && (ageFilter === "Alle" || player.ageGroup === ageFilter) && player.name.toLowerCase().includes(query.toLowerCase()))
    .sort((left, right) => (responseOrder[event.responses[left.id] as keyof typeof responseOrder] ?? 2) - (responseOrder[event.responses[right.id] as keyof typeof responseOrder] ?? 2) || left.name.localeCompare(right.name, "de"));
...
function TournamentCreateDialog({ settings, playerCount, busy, onClose, onSave }: { settings: ClubSettings; playerCount: number; busy: boolean; onClose: () => void; onSave: (event: ClubEvent) => void }) {
  const [form, setForm] = useState({ title: "", date: today(), location: "", startTime: "10:00", endTime: "13:00", meetingTime: "09:15", autoSetPlayersPresent: false });
  const [error, setError] = useState("");
  return <div className="modal-backdrop squad-editor-backdrop" onMouseDown={onClose}><form className="tournament-create" role="dialog" aria-modal="true" aria-labelledby="create-tournament-title" onMouseDown={(event) => event.stopPropagation()} onSubmit={(event_) => { event_.preventDefault(); if (form.date < today()) return setError("Das Turnierdatum darf nicht in der Vergangenheit liegen."); onSave({ id: `event-${Date.now()}`, type: "tournament", title: form.title.trim(), date: form.date, location: form.location.trim(), address: "", startTime: form.startTime, endTime: form.endTime, meetingTime: form.meetingTime, description: "", trainerNote: "", autoSetPlayersPresent: form.autoSetPlayersPresent, maxParticipants: form.autoSetPlayersPresent ? Math.max(settings.defaultTournamentCapacity, playerCount) : settings.defaultTournamentCapacity, responses: {} }); }}><header><div><span className="eyebrow">NEUES FUSSBALLTURNIER</span><h2 id="create-tournament-title">Turnier anlegen</h2></div><button type="button" onClick={onClose}><X /></button></header><label><span>Name des Turniers</span><input required maxLength={160} value={form.title} onChange={(e) => setForm({ ...form, title: e.target.value })} placeholder="z. B. Kinderfußball-Festival" /></label><div className="form-row"><label><span>Datum</span><input required min={today()} type="date" value={form.date} onChange={(e) => setForm({ ...form, date: e.target.value })} /></label><label><span>Ort</span><input required maxLength={180} value={form.location} onChange={(e) => setForm({ ...form, location: e.target.value })} placeholder="Sportanlage / Verein" /></label></div><div className="form-row tournament-times"><label><span>Treffen</span><input type="time" value={form.meetingTime} onChange={(e) => setForm({ ...form, meetingTime: e.target.value })} /></label><label><span>Beginn</span><input type="time" value={form.startTime} onChange={(e) => setForm({ ...form, startTime: e.target.value })} /></label><label><span>Ende</span><input type="time" value={form.endTime} onChange={(e) => setForm({ ...form, endTime: e.target.value })} /></label></div>{settings.attendanceEnabled && <fieldset className="event-attendance-mode"><legend>Wie soll die Teilnahme starten?</legend><div><button type="button" className={!form.autoSetPlayersPresent ? "active" : ""} aria-pressed={!form.autoSetPlayersPresent} onClick={() => setForm({ ...form, autoSetPlayersPresent: false })}><span><strong>Rückmeldung einholen</strong><small>Spieler sagen selbst zu. Persönliche Standards werden berücksichtigt.</small></span>{!form.autoSetPlayersPresent && <Check />}</button><button type="button" className={form.autoSetPlayersPresent ? "active" : ""} aria-pressed={form.autoSetPlayersPresent} onClick={() => setForm({ ...form, autoSetPlayersPresent: true })}><span><strong>Absage erforderlich</strong><small>Alle starten als dabei und melden sich nur, wenn sie fehlen.</small></span>{form.autoSetPlayersPresent && <Check />}</button></div></fieldset>}{error && <div className="squad-editor-error">{error}</div>}<footer><button type="button" onClick={onClose}>Abbrechen</button><button className="primary" disabled={busy} type="submit"><Plus /> {busy ? "Wird angelegt …" : "Turnier anlegen"}</button></footer></form></div>;
}

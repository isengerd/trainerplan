...
export function EmailConfirmation({ token }: { token: string }) {
  ...
  return <main className="login-page email-confirmation-page"><section className="login-brand"><span className="brand-mark"><Shield /></span><span className="eyebrow">NEXTSESSION KIDS!</span><h1>Sicher.<br />Bestätigt.</h1><p>E-Mail-Adressen werden erst nach dem Klick auf den persönlichen Bestätigungslink übernommen.</p></section><section className="login-panel"><div className="email-confirmation-card"><span className={`email-confirmation-icon ${state}`}>{state === "loading" ? <LoaderCircle /> : state === "success" ? <Check /> : <X />}</span><span className="eyebrow">E-MAIL-ÄNDERUNG</span><h2>{state === "loading" ? "Wird geprüft" : state === "success" ? "Adresse bestätigt" : "Bestätigung fehlgeschlagen"}</h2><p>{message}</p>{state !== "loading" && <a href="/"><Mail /> Zur Anmeldung</a>}</div></section></main>;
}

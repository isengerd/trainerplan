...
export async function getTournamentSquads(eventId: string, playerId?: string) {
  ...
  return squads.filter((squad) => !playerId || squad.players.some((player) => player.playerId === playerId)).map((squad) => ({ id: squad.id, eventId, name: squad.name, trainerId: squad.trainerId, playerIds: playerId ? [] : squad.players.map((player) => player.playerId) }));
}

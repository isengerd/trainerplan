...
export async function sendPushToUsers(input: { userIds: string[]; title: string; body: string; eventId?: string }) {
  ...
  if (!messaging || !input.userIds.length) return { sent: 0, devices: 0, configured: Boolean(messaging), failureCodes: [] as string[] };
  ...
  return { sent, devices: devices.length, configured: true, failureCodes: [...failureCodes] };
}

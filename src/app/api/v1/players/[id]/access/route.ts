...
async function managedChild(request: NextRequest, id: string) {
  ...
  const membership = await prisma.membership.findFirst({ where: { userId: id, clubId: scope.clubId, teamId: scope.teamId, status: "active", role: "player" } });
...
export async function POST(request: NextRequest, context: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    await prisma.invitation.deleteMany({ where: { managedPlayerId: id, role: "guardian", clubId: scope.clubId, teamId: scope.teamId, acceptedAt: null, email } });
...
export async function DELETE(request: NextRequest, context: { params: Promise<{ id: string }> }) {
  try {
    ...
    if (invitationId) {
      await prisma.invitation.deleteMany({ where: { id: invitationId, managedPlayerId: id, role: "guardian", clubId: scope.clubId, teamId: scope.teamId, acceptedAt: null } });
    } else {
